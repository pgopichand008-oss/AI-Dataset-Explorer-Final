...
from config import get_gemini_client, get_gemini_api_key
...
def _local_fallback_report(
    ...
    lines.append(
        "The dataset has sufficient structure for exploratory analysis, but substantial missing data and several quality risks should be addressed before reliable predictive modeling."
    )
    ...
    if isinstance(quality_findings, list):
        for finding in quality_findings[:7]:
            if isinstance(finding, dict):
                ...
                lines.append(f"- **{issue}** (Severity: {sev}): {det}")
            else:
                lines.append(f"- {_safe_text(finding)}")
    elif isinstance(quality_findings, dict):
        for key, value in list(quality_findings.items())[:7]:
            lines.append(f"- {key}: {_safe_text(value)}")
    else:
        lines.append("- No major quality findings were supplied.")
    ...
    if isinstance(quality_findings, list):
        for item in quality_findings:
            if isinstance(item, dict):
                issue = item.get("Issue", item.get("finding", "Quality Issue"))
        ...
        lines.append(str(quality_findings))
    else:
        lines.append("Dataset completeness and schema integrity have been evaluated across all attributes.")
    ...
    if ml_readiness is not None:
        if ml_readiness >= 80:
            readiness_text = "The dataset appears mostly ready for further machine-learning analysis, subject to normal preprocessing and validation."
        elif ml_readiness >= 60:
            readiness_text = "The dataset may be usable for machine-learning analysis, but several preprocessing or quality issues should be reviewed first."
        else:
            readiness_text = "The dataset requires additional preparation before machine-learning analysis."
        lines.append(readiness_text)
    else:
        lines.append("ML readiness score was not supplied.")
    ...
    lines.append("• Column renaming and standardizing data types will improve schema integrity.")
    lines.append("• Imputing missing numerical values will expand usable rows for modeling.")
    ...
    lines.append("• High missing value percentage in key columns restricts immediate baseline model training.")
    lines.append("• High cardinality in categorical variables requires feature encoding prior to model fitting.")
...
def generate_ai_report(
    ...
    **kwargs,
    ...
    # --------------------------------------------------------
    # GEMINI FAILED — USE LOCAL FALLBACK (CLEAN)
    # --------------------------------------------------------
